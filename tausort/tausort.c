...
void initialize(void)
{
  ...
  for (nu = 0; nu <= Nnu - 1; nu++)
  {
    ...
    numid[nu] = .5e0 * Cvac * (1. / wbeg[nu] + 1. / wend[nu]) * 1.e07;
    ...
    for (j = 0; j <= NT - 1; j++)
    {
      ...
      dBdT[nu][j] = 2.e0 * pow(H, 2) / (k_B * pow(Cvac, 2)) * pow(numid[nu], 4) / pow(exp(TENLOG * tab_T[j]), 2) * 1.e0 / (exp(H * numid[nu] / (k_B * exp(TENLOG * tab_T[j]))) + exp(-1.e0 * H * numid[nu] / (k_B * exp(TENLOG * tab_T[j]))) - 2.e0);
    }
  }
      ...
      if ((lgT >= tab_T[l]) && (lgT < tab_T[l + 1]))
      {
        for (m = 1; m <= Np - 2; m++)
        {
          if ((lgp >= tab_p[m]) && (lgp < tab_p[m + 1]))
          {
            for (nu = 0; nu <= Nnu - 1; nu++)
            {
              kcont[k][nu] = 1.0 / (tab_T[l + 1] - tab_T[l]) * 1.0 / (tab_p[m + 1] - tab_p[m]) * ((tab_T[l + 1] - lgT) * (tab_p[m + 1] - lgp) * log10(kcont_pT[nu][l][m]) + (lgT - tab_T[l]) * (tab_p[m + 1] - lgp) * log10(kcont_pT[nu][l + 1][m]) + (tab_T[l + 1] - lgT) * (lgp - tab_p[m]) * log10(kcont_pT[nu][l][m + 1]) + (lgT - tab_T[l]) * (lgp - tab_p[m]) * log10(kcont_pT[nu][l + 1][m + 1]));
              kcont[k][nu] = exp(TENLOG * kcont[k][nu]);

              scont[k][nu] = 1.0 / (tab_T[l + 1] - tab_T[l]) * 1.0 / (tab_p[m + 1] - tab_p[m]) * ((tab_T[l + 1] - lgT) * (tab_p[m + 1] - lgp) * log10(scont_pT[nu][l][m]) + (lgT - tab_T[l]) * (tab_p[m + 1] - lgp) * log10(scont_pT[nu][l + 1][m]) + (tab_T[l + 1] - lgT) * (lgp - tab_p[m]) * log10(scont_pT[nu][l][m + 1]) + (lgT - tab_T[l]) * (lgp - tab_p[m]) * log10(scont_pT[nu][l + 1][m + 1]));
              ...
              {
                line_opacity[nu][bin][k] = 1. / (tab_T[l + 1] - tab_T[l]) * 1. / (tab_p[m + 1] - tab_p[m]) * ((tab_T[l + 1] - lgT) * (tab_p[m + 1] - lgp) * ODF[nu][l][m][bin] + (lgT - tab_T[l]) * (tab_p[m + 1] - lgp) * ODF[nu][l + 1][m][bin] + (tab_T[l + 1] - lgT) * (lgp - tab_p[m]) * ODF[nu][l][m + 1][bin] + (lgT - tab_T[l]) * (lgp - tab_p[m]) * ODF[nu][l + 1][m + 1][bin]);
    ...
    for (bin = 0; bin <= Nbin - 1; bin++)
    {
      ...
      for (i = 1; i <= KMAX - 1; i++)
      {
        tau_nu[nu][bin][i] = tau_nu[nu][bin][i - 1] + dzi[i][1] *
                                                          ((1.e0 / 3.e0) * kap[nu][bin][i] * rho[i] + (1.e0 / 6.e0) * kap[nu][bin][i - 1] * rho[i] + (1.e0 / 6.e0) * kap[nu][bin][i] * rho[i - 1] + (1.e0 / 3.e0) * kap[nu][bin][i - 1] * rho[i - 1]);
      }
    }
  }
  ...
  for (i = 1; i <= KMAX - 1; i++)
  {
    tau5000[i] = tau5000[i - 1] + dzi[i][1] *
                                      ((1.e0 / 3.e0) * acont[i][i_5000] * rho[i] + (1.e0 / 6.e0) * acont[i - 1][i_5000] * rho[i] + (1.e0 / 6.e0) * acont[i][i_5000] * rho[i - 1] + (1.e0 / 3.e0) * acont[i - 1][i_5000] * rho[i - 1]);
    ...
    for (int p = 0; p <= Np - 1; p++)
    {
      for (int band = 0; band <= Nbands - 1; band++)
      {
        ...
        if (ROSS || GREY)
          ...
          kap_mean[band][t][p] = log(kap_pl[band][t][p] * pow(2.e0, -1.e0 * tau_i / TAU1_2[band]) + kap_ro[band][t][p] * (1.e0 - pow(2.e0, -1.e0 * tau_i / TAU1_2[band])));
      }
  ...
  for (int j = 0; j <= NT - 1; j++)
  {
    for (int k = 0; k <= Np - 1; k++)
    {
      for (int nu = 0; nu <= Nnu - 1; nu++)
      {
        for (int bin = 0; bin <= Nbin - 1; bin++)
        {
          kap_mean[nu * Nbin + bin][j][k] = log(pow(10, (float)1.e-3 * ODF[nu][j][k][bin]) + acont_pT[nu][j][k]);
        }
      }
      // Tau5000 bin for reference output
